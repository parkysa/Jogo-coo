...
public class GameLib {
    ...
    public static void drawExplosion(double x, double y, double alpha){
        try {
            // Garantir que alpha está entre 0 e 1
            alpha = Math.max(0.0, Math.min(1.0, alpha));
            
            int p = 5;
            int r = (int) Math.max(0, Math.min(255, 255 - Math.pow(alpha, p) * 255));
            int g_color = (int) Math.max(0, Math.min(255, 128 - Math.pow(alpha, p) * 128));
            int b = 0;

            GameLib.setColor(new Color(r, g_color, b));
            double radius = alpha * alpha * 40;
            GameLib.drawCircle(x, y, radius);
            GameLib.drawCircle(x, y, radius + 1);
        } catch (Exception e) {
            // Fallback para cor simples
            GameLib.setColor(Color.ORANGE);
            GameLib.drawCircle(x, y, 10);
        }
    }
